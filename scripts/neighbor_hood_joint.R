...
write.table(df_ddi_fdi_ffi_info_sp_algo, "My Doc/Final_Calculation_Information_NB_dataset_2_joint.csv", row.names = FALSE, col.names = FALSE, sep=",") 
...
neighbor_final_output <- function(main_dataset_for_neighbor_algo) {
  ...
  write.csv(cn_edgelist, "My Doc/new_link_prediction_NB_CN_dataset_2_joint.csv", row.names = FALSE) 
  ...
  write.csv(aa_edgelist, "My Doc/new_link_prediction_NB_AA_dataset_2_joint.csv", row.names = FALSE) 
  ...
  write.csv(l3_edgelist, "My Doc/new_link_prediction_NB_L3_dataset_2_joint.csv", row.names = FALSE) 
  ...
  write.csv(ra_edgelist, "My Doc/new_link_prediction_NB_RA_dataset_2_joint.csv", row.names = FALSE) 
  
}

# Training the dataset using Neighbor Methods

train_dataset_for_neighbor <- read.csv("My Doc/dataset_2_joint.csv", header = TRUE)
...
Calculation_Information_function <- function(nb_methods)
{
  ...
  all_data_ddi_fdi_ffi_info_sp_algo <- read.csv(file = paste0("My Doc/new_link_prediction_NB_", nb_method, 
  ...
  write.csv(all_ddi, file = paste0("My Doc/new_link_prediction_NB_", nb_method,
  ...
  write.csv(df_remove_ff, file = paste0("My Doc/new_link_prediction_NB_", nb_method,
  ...
  df_ddi_fdi_ffi_info_sp_algo <-  read.csv(file = paste0("My Doc/Final_Calculation_Information_NB_", 
                                                         "dataset_2_joint",".csv", sep=""))
  
  df_ddi_fdi_ffi_info_sp_algo <- rbind(df_ddi_fdi_ffi_info_sp_algo, temp_df_ddi_fdi_ffi_info_sp_algo) 
  
  write.csv(df_ddi_fdi_ffi_info_sp_algo, file = paste0("My Doc/Final_Calculation_Information_NB_", 
