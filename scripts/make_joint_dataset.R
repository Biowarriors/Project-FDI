...
making_joint_dataset <- function(main_dataset_disjoint){
  ...
  df$new_ssp <-NULL
  write.csv(df, "My Doc/dataset_2_joint.csv", row.names = FALSE) 
  }
  else{
    print("Dataset dose not contain subgraphs. So, Disjoint and Joint they are same graph")
    write.csv(g, "My Doc/dataset_2_joint.csv", row.names = FALSE) 
  }
...
main_dataset_disjoint <- read.csv("My Doc/dataset_1_disjoint.csv", header = TRUE)
